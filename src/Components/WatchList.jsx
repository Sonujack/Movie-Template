...
function WatchList({ watchList, setWatchList, handleRemoveWatchList }) {
  ...
  return (
    <>
      <div className='flex justify-center flex-wrap m-4 gap-10'>
        {genreList.map((genre)=>{
          return <div key={genre} onClick={()=>handleFilter(genre)}className={currGenre==genre?'bg-blue-400 flex justify-center h-[3rem] w-[9rem] rounded-xl items-center text-white font-bold hover:cursor-pointer':'bg-gray-400 flex justify-center h-[3rem] w-[9rem] rounded-xl items-center text-white font-bold hover:cursor-pointer'}>{genre}</div>
        })}
   </div>
      ...
      <div className="flex justify-center">
        <input 
          ...
          type="text" 
          placeholder='Search WatchList Movie Name...' 
        />
      </div>
            ...
            {watchList.filter((movieObj)=>{
              ...
              .map((movieObj) => (
                <tr key={movieObj.id} className="border-b-2">
                  ...
                  <td onClick={() => handleRemoveWatchList(movieObj)} className="cursor-pointer text-red-500 font-bold">Delete</td>
                </tr>
              ))}
          </tbody>
        </table>
